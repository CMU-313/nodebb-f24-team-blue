...
describe('Topic\'s', () => {
	...
	describe('.post', () => {
		...
		it('should create a new anonymous topic if anonymous is true', (done) => {
			topics.post({
				uid: topic.userId,
				title: topic.title,
				content: topic.content,
		...
		it('should create a new regulat topic if anonymous is false', (done) => {
			topics.post({
				uid: topic.userId,
				title: topic.title,
				content: topic.content,
	...
	describe('Get methods', () => {
		...
		describe('.getsearchTopics', () => {
			let searchTopic;

			before(async () => {
				searchTopic = await topics.post({
					uid: adminUid,
					cid: categoryObj.cid,
					title: 'Search Title',
					content: 'Search Content',
				});
				const topicKey = `topic:${searchTopic.topicData.tid}`;
				await redis.del(topicKey);

				await redis.hset(topicKey, {
					tid: searchTopic.topicData.tid.toString(),
					uid: searchTopic.topicData.uid.toString(),
					cid: searchTopic.topicData.cid.toString(),
					mainPid: searchTopic.topicData.mainPid.toString(),
					title: searchTopic.topicData.title,
					slug: searchTopic.topicData.slug,
					timestamp: searchTopic.topicData.timestamp.toString(),
					lastposttime: searchTopic.topicData.lastposttime.toString(),
					postcount: searchTopic.topicData.postcount.toString(),
					viewcount: searchTopic.topicData.viewcount.toString(),
					postercount: searchTopic.topicData.postercount.toString(),
				});
			});

			it('should get topics that match the search term "Search" in the category', async () => {
				const uid = adminUid;
				const { cid } = categoryObj;
				const searchTerm = 'Search';
				const response = await apiCategories.searchTopics({ uid }, { searchTerm, cid });

				assert(response.topics);
				assert(Array.isArray(response.topics));
				assert(response.topics.length > 0);

				// Check that all topics in the returned array contain the search term
				response.topics.forEach((topic) => {
					assert(topic.title.includes(searchTerm));
			...
			after(async () => {
				const topicKey = `topic:${searchTopic.topicData.tid}`;
				await redis.del(topicKey);
				await topics.delete(searchTopic.topicData.tid);
			});
		});

		describe('.getanonymousTopics', () => {
			let anonymousTopic;

			// Before and after setup generated by ChatGPT
			before(async () => {
				anonymousTopic = await topics.post({
					uid: adminUid,
					cid: categoryObj.cid,
				...
				await redis.hset(topicKey, {
					tid: anonymousTopic.topicData.tid.toString(),
					uid: anonymousTopic.topicData.uid.toString(),
					cid: anonymousTopic.topicData.cid.toString(),
					mainPid: anonymousTopic.topicData.mainPid.toString(),
					title: anonymousTopic.topicData.title,
					slug: anonymousTopic.topicData.slug,
					timestamp: anonymousTopic.topicData.timestamp.toString(),
					lastposttime: anonymousTopic.topicData.lastposttime.toString(),
					postcount: anonymousTopic.topicData.postcount.toString(),
					viewcount: anonymousTopic.topicData.viewcount.toString(),
					postercount: anonymousTopic.topicData.postercount.toString(),
					anonymous: anonymousTopic.topicData.anonymous.toString(),
				});
			});
			...
			it('should get anonymous topics from the category', async () => {
				...
				// Check that all topics in the returned array are anonymous
				response.topics.forEach((topic) => {
			...
			after(async () => {
				const topicKey = `topic:${anonymousTopic.topicData.tid}`;
				await redis.del(topicKey);
				await topics.delete(anonymousTopic.topicData.tid);
			});
		});
